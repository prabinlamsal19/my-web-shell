  ...
  switch(input) {
    ...
    case 'cv':
      ...
      setTimeout(() => {
        ...
        link.href = '/src/assets/PrabinLamsal-NextFlutter.pdf';
        link.download = 'PrabinLamsal-NextFlutter.pdf';
...
const initEventListeners = () => {
  ...
  if(PRE_USER) {
    ...
    const fakePrompt = `<span style="color: ${colors.prompt.user}">${command.username}</span><span style="color: ${colors.prompt.default}">@</span><span style="color: ${colors.prompt.host}">${command.hostname}</span><span style="color: ${colors.prompt.default}">:$ ~ </span><span style="color: ${colors.prompt.input}">neofetch</span>`;
    writeLines([fakePrompt, ...BANNER]);
  });
